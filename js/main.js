...
var marker1 = L.marker([48.20467, 16.36911], {icon: defaultIcon}).addTo(mainMap);
var marker2 = L.marker([48.20320, 16.36918], {icon: defaultIcon}).addTo(mainMap);

marker1.on('click', onMarker1Click);
marker2.on('click', onMarker2Click);

function onMarker1Click(e) {
  marker2.setIcon(defaultIcon); // TODO: for all markers
  e.target.setIcon(activeIcon);
  mainMap.panTo(e.target.getLatLng());
  $('#markerStaatsoper').hide();
  $('#markerPhilipphof').show();
}

function onMarker2Click(e) {
  marker1.setIcon(defaultIcon); // TODO: for all markers
  e.target.setIcon(activeIcon);
  mainMap.panTo(e.target.getLatLng());
  $('#markerPhilipphof').hide();
  $('#markerStaatsoper').show();
}
